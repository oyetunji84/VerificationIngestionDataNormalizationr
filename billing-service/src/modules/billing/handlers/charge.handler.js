...
const chargeWalletHandler = async (call, callback) => {
  const { organization_id, service_type, idempotency_key } = call.request;
  ...
  try {
    ...
    callback(null, {
      cost_kobo: Math.round(result.cost * 100),
...
module.exports = { chargeWalletHandler };
