...
const createMiddleware=()=> {
  return async (req, res, next) => {
    const apiKey = req.header("x-api-key") || req.header("X-API-KEY") || req.headers["x-api-key"];
    ...
    try {
      const result = await consume(apiKey, endpoint, config.capacity, config.leakRate);
      ...
      if (result.error) {
        ...
        console.log(`Rate limiting degraded for ${endpoint} - Redis unavailable`);        
        return next();
      }
      ...
      if (!result.allowed) {
        ...
        res.setHeader('Retry-After', result.retryAfter);
         throw new RateLimitExceededError(`You have exceeded the rate limit of ${result.limit} requests per minute for this endpoint. Please try again after ${Math.ceil(result.retryAfter)} seconds.`);
      }
      next();
...
async function getRateLimitInfo(redis, apiKey, endpoint) {
  ...
  const state = await getState(apiKey, normalizedEndpoint, config.capacity, config.leakRate);
